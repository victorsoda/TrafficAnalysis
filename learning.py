...
def pursuit(data, n_iterations=20):
    n_rows, n_cols = np.array(data).shape
    table_of_info_gains = np.zeros((n_iterations, n_cols+10))
    ...
    for iteration in range(n_iterations):
        ...
        next_best_output = 0
        next_best_f = []
        for output_type in [0, 1, 2, 3]:    # 分别对应ΔF为0->0，1->0，0->1，1->1
            indices10 = 2 * output_type - 2
            ...
            for action_index in range(1, n_cols-1):     # 检查每个action和每个output type
                ...
                for i in range(len(best_actions)):
                    if best_actions[i] == action_index:
                        tmp_index = i
                if best_output[tmp_index] != output_type:
                    # 该动作对应的真实分布f
        ...
        f = next_best_f
        new_causal_effect = (f[3] / (f[3] + f[1]) - f[2] / (f[2] + f[0])) \
                            / (1 - f[2] / (f[2] + f[0]))
